...
namespace osnE.WindowsHooks
{
    ...
    private IShellLinkW m_Link;
#else
        private NativeMethods.IShellLinkA m_Link;
#endif
      ...
      m_Link = ( IShellLinkW) new ShellLink();
#else
            m_Link = (NativeMethods.IShellLinkA)new NativeMethods.ShellLink();
#endif

            if (File.Exists(linkPath))
            {
                pf = (NativeMethods.IPersistFile)m_Link;
                pf.Load(linkPath, 0);
            }
...
#else
                NativeMethods.WIN32_FIND_DATAA wfd = new NativeMethods.WIN32_FIND_DATAA();
#endif
                StringBuilder sb = new StringBuilder(MAX_PATH);

                m_Link.GetPath(sb, sb.Capacity, out wfd, NativeMethods.SLGP_FLAGS.SLGP_UNCPRIORITY);
                return sb.ToString();
            }
        ...
        public void Save()
        {
            NativeMethods.IPersistFile pf = (NativeMethods.IPersistFile)m_Link;
